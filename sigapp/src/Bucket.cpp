...
Bucket::Bucket(float model_scale) {
	planeClass = new SnGroup;
...
void Bucket::import_model() {
	...
	if (carr->model()->load("../model/bucket.obj")) {}
...
void Bucket::run_animation(float time) {
...
int Bucket::handle_keyboard(const GsEvent& e) {
	GsMat trans, scaling;
	scaling.scaling(currentScale);
	switch (e.key)
	{
...
//void Bucket::updateShadows(GsVec pos, GsMat trans, GsMat shadowMat, GsMat rot) {
//	GsMat oPos, scaling;
//	scaling.scaling(currentScale);
...
void Bucket::set_position(GsVec pos) {
...
void Bucket::setScaling(float sca) {
	GsMat scaling, position;
	position.translation(currPos);
	currentScale = sca;
	scaling.scaling(currentScale);
