...
void Town::import_model() {
	...
	if (carr->model()->load("../model/cLand2.obj")) {}
...
void Town::run_animation(float time) {
...
int Town::handle_keyboard(const GsEvent& e) {
	GsMat trans, scaling;
	scaling.scaling(currentScale);
	switch (e.key)
	{
...
//void Town::updateShadows(GsVec pos, GsMat trans, GsMat shadowMat, GsMat rot) {
//	GsMat oPos, scaling;
//	scaling.scaling(currentScale);
...
void Town::set_position(GsVec pos) {
...
void Town::setScaling(float sca) {
	GsMat scaling, position;
	position.translation(currPos);
	currentScale = sca;
	scaling.scaling(currentScale);
