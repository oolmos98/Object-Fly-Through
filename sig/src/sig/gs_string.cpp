...
GsString::GsString ( GsOutput& o )
{
	GS_TRACE1 ( "Output Constructor" );
	o.init(*this);
}
...
{
	GS_TRACE3 ( "start set: [" << (st?st:"(null)")<<"]" );
...
# define BUFSIZE 48
# define SNPRINTF(st,v) snprintf(&(_data[len]),BUFSIZE,st,v)
...
GsStringf::GsStringf ( const char *fmt, ... )
 {
   if ( !fmt ) return;
