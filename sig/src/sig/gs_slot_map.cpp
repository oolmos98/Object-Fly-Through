...
int GsSlotMapBase::nextid () const
 {
   return _freepos.size()>0? _freepos.top() : _data.size();
 }

int GsSlotMapBase::insert ( void* pt )
 {
   int i;
...
void* GsSlotMapBase::extract ( int i )
 {
...
bool GsSlotMapBase::remove_gaps ( GsArray<int>& newindices )
 {
   int i, j;
