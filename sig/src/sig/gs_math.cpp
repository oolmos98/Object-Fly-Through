...
//-------------------------------------------------------------------
// Functions to solve polynomials of 2nd, 3rd and 4th degree; from graphics gems.
// Thanks to Lodovico Ferrari (1540), Gerolamo Cardano (1545),
// Valmes (1486) who maybe was burned at the stake for claiming a solution, and
// Abel–Ruffini (1824) for showing that higher order polynomials cannot be solved.
//-------------------------------------------------------------------

# define aMAXFLOAT	3.40282346E+38F
...
int gs_solve_cubic_polynomial ( double c[4], double s[3] )
 {
   ...
   if ( aISZERO(D) )
	...
	{ double phi = 1.0/3 * acos ( -q/sqrt(-cb_p) );
	  double t = 2 * sqrt(-p);
	  s[0] =   t * cos(phi);
	  s[1] = - t * cos(phi + GS_PI / 3);
	  s[2] = - t * cos(phi - GS_PI / 3);
	  num = 3;
	}
...
int gs_solve_quartic_polynomial ( double c[5], double s[4] )
 {
   ...
   if ( aISZERO(r) ) // no absolute term: y(y^3 + py + q) = 0
	{ coeffs[0] = q;
	  ...
	  coeffs[2] = 0;
	  coeffs[3] = 1;
	  num = gs_solve_cubic_polynomial ( coeffs, s );
	  s[num++] = 0;
	}
	...
	{ coeffs[0] = 1.0/2 * r * p - 1.0/8 * q * q;
	  ...
	  coeffs[2] = - 1.0/2 * p;
	  coeffs[3] = 1;
	  gs_solve_cubic_polynomial ( coeffs, s );
	  ...
	  coeffs[1] = q < 0 ? -v : v;
	  coeffs[2] = 1;
	  num = gs_solve_quadratic_polynomial ( coeffs, s );

	  coeffs[0]= z + u;
	  coeffs[1] = q < 0 ? v : -v;
	  coeffs[2] = 1;
	  num += gs_solve_quadratic_polynomial ( coeffs, s+num );
	}
...
void gs_get_closest_on_ellipse ( float a, float b, float& x, float& y )
 {
   ...
   int nb = gs_solve_quartic_polynomial ( c, s );
