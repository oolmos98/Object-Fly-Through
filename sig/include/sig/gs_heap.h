...
{  protected :
   ...
   public :
	...
	/*! Returns non-const reference for elem i (0<=i<size) */
	X& elemref ( int i ) { return _heap[i].e; }
...
/*! \class GsHeapPt gs_heap.h
	\brief GsHeap extension for allocated pointers - not used or tested yet */
template <typename X, typename Y>
class GsHeapPt : protected GsHeap<X*,Y>
{  public:
   ...
   ~GsHeapPt () { init(); }
	...
	/*! Initializes as an empty heap, deleting all elements stored in the heap */
	void init () { while (size()) delete GsHeap<X*,Y>::_heap.pop().e; }

	/*! Get a pointer to the top element of the the heap,
		which is always the element with lowest cost. */
	X* top () const { return GsHeap<X*,Y>::top(); }

	/*! Get the lowest cost in the heap,
		which is always the cost of the top element. */
	Y lowest_cost () const { return GsHeap<X*,Y>::lowest_cost(); }

    /*! Removes the top element of the heap and deletes it */
    void remove () { delete GsHeap<X*,Y>::top(); GsHeap<X*,Y>::remove(); }

    /*! Removes the top element of the heap and returns its pointer. The user will be
	    then responsible to later delete the returned object with operator delete */
    X* extract () { X* pt=GsHeap<X*,Y>::top(); GsHeap<X*,Y>::remove(); return pt; }

    /*! Returns true if the heap is empty, false otherwise. */
    bool empty () const { return GsHeap<X*,Y>::empty(); }

    /*! Returns the number of elements in the queue. */
    int size () const { return GsHeap<X*,Y>::size(); }

    /*! Compress the internal heap array */
    void compress () { GsHeap<X*,Y>::compress(); }
};
