...
/** \file gs.h 
 * Main header file of the toolkit
 *
 \code
 *************************************************************************
 *
 ...
 *   - Geometric classes are in 3D, otherwise a letter is used to indicate
 *	  the dimension, e.g.: GsVec, GsVec2
 *
 *************************************************************************
 *
 ...
 * global enums					: gsMsgOk (enums should be defined in class scope whenever possible)
...
/*! GsCharPt is a class containing a single char pointer pointing to a
	c-like string, which is maintained by the class with proper	constructors,
	a destructor, and additional methods, all implemented inline. */
struct GsCharPt
{	char* pt;
	GsCharPt () { pt=0; }
	GsCharPt ( const char* tocopy ) { pt=gs_string_new(tocopy); }
   ~GsCharPt () { delete[] pt; }
	int len () const { int l=0; while(pt[l]) l++; return l; }
	void set ( const char* tocopy ) { gs_string_set(pt,tocopy); }
	void renew ( int size ) { gs_string_renew(pt,size); }
	void append ( const char* toadd ) { gs_string_append(pt,toadd); }
	void operator = ( const char* tocopy ) { set(tocopy); }
	bool operator == ( const char* tocmp ) { return gs_comparecs(pt,tocmp)==0; }
...
/*! Returns true if given file name is a directory and false otherwise
	False is returned if fname is null or if fname[0] is 0. */
...
/*! Returns the size in bytes of file fname, or 0 if fname does not exist.
	If fname is null (default) the info read by the latest call to 
	gs_exists() or gs_isdir() is used. */
gsuint gs_size ( const char* fname=0 );

/*! Returns the size in bytes of fname as a long long suitable for large files,
	or 0 if fname does not exist. If fname is null (default) the info read by
	the latest call to gs_exists() or gs_isdir() is used. */
unsigned long long gs_sizel ( const char* fname=0 );

/*! Returns the modification time of the file as a Unix timestamp
	(number of seconds since the start of 1970 in GMT). Returns 0
	if the file does not exist. If fname is null (default) the info
	read by the latest call to gs_exists() or gs_isdir() is used. */
...
# define GS_ISUPPER(c) ( (c)>='A'&&(c)<='Z' )

/*! Macro that returns if c is lower case. */
# define GS_ISLOWER(c) ( (c)>='a'&&(c)<='z' )
...
/*! Macro that returns the minimum value of the two arguments. */
# define GS_MIN_MAX(a,b,min,max) if((a)<(b)) {min=a;max=b;} else {min=b;max=a;}
...
/*! Sets off the given bit of the flag. */ 
# define GS_FLAG_OFF(flg,bit) flg&=~bit
...
# define GS_FLAG_SET(flg,bit,val) flg = val? (flg)|(bit) : (flg)&~(bit)
