...
{  public :
	...
	enum Type { TypeStdout,  //!< Output is directed to the console
				...
				TypeStderr   //!< Output is directed to the console via stderr
			  };

   private :
	gscenum _type;
	gscbool _auto_console;
	char _margin_char;
	int  _margin;
   ...
   ~GsOutput ();
	...
	/*! Closes current ouput and makes it be of type stdout or stderr as specified by t. */
	void init ( Type t=TypeStdout );
	...
	bool auto_console () const { return _auto_console==1; }
	...
	/*! Outputs a formated string to the connected device. The format string is the same
		as for the printf() function. An internal 192 bytes buffer is used to translate
		the format string and longer strings are truncated. */
	void putf ( const char *fmt, ... );

	/*! Calls stdlib exit(code). If there is an active attached console, it will first wait
		for a key press (in Windows only). */
	void exit ( int code=0 );

	/*! If there is an active attached console, it will wait for a key press (Windows only).
		If msg is null, the default message ("Press Enter to continue...") is showed. */
	void pause ( const char* msg=0 );
	...
	/*! Outputs C-like arguments to a main function */
	...
	/*! Outputs a string using the put() method. */
	friend GsOutput& operator<< ( GsOutput& o, const GsCharPt& s ) { o.put(s.pt); return o; }
...
/*! The global output gsout is used in the sig toolkit as the default output of messages.
	By default it sends messages to stdout and it can be redirected as needed. */
extern GsOutput gsout;

/*! This class is available as a trick to call safewrite() with operators,
	for example: out << "name " << GsSafeWrite(_name) << gsnl; */
struct GsSafeWrite
{	const char* s;
