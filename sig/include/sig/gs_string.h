...
{ private:
   ...
   public:
	...
	/*! Constructs with len+1 size and with a 0 at position len. */
	GsString ( int len );

	/*! This constructor connects the string to the given output. */
	GsString ( GsOutput& o );
	...
	/*! Makes GsString empty, having the internal buffer freed. */
	void clear () { set(0); }

	/*! Sets GsString to contain the given formatted string, using vsnprintf()
		with a limit of 192, and then set() is called for the result. */
	void setf ( const char *fmt, ... );

	/*! Returns the length of the string (not counting the ending null character) */
	int len () const { return _data.size(); }

	/*! Ensures the string has sufficient size to hold a string of len l.
		The string length (and array size) will be l, and a null character is placed
		at position l, which stays in the reserved allocated portion of the array. */
	void len ( int l );
	...
	operator const char* () const { return _data.size()? _data.pt():_empty; }

	/*! Ok, when type casts do not work, access pointer via this method */
	const char* pt () const { return _data.size()? _data.pt():_empty; }
	...
	/*! Takes ownership of the data in s, and then makes s an empty string. */
	void adopt ( GsString &s ) { _data.adopt(s._data); }

	/*! Takes ownership of the data in s, which will be then managed by GsString
		using malloc/realloc/free functions. */
	void adopt ( char* s, int len ) { _data.adopt(s,len,len+1); }

	/*! Makes GsString become empty, and assigns the internal buffer to s. Returns
		the string s, which will never be 0, and which will never point to the
	...
	GsString& operator << ( int i );	//!< converts and appends i
	GsString& operator << ( float f );  //!< converts and appends f
	GsString& operator << ( double d ) { return (*this)<<(float)d; } //!< converts and appends d
	GsString& operator << ( char c );   //!< converts and appends character c 

	/*! Write the string in the output making sure it can be read again as a string,
		by enclosing the string with double quotes if needed */
	GsOutput& safewrite ( GsOutput& o ) const { o.safewrite(*this); return o; }
...
/*! GsStringf is a convenience class that derives GsString in order to provide a
	variable-argument formatted string constructor. In this way the original
	constructor of GsString remains efficient for non-formated string constructors. */
class GsStringf : public GsString
{	public:
	/*! Initializes GsStringf to contain the given formatted string, using vsnprintf()
		with a limit of 192, and then set() is called for the result. */
	GsStringf ( const char *fmt, ... );
};
...
/*! Search for a file name at the end of the path and put it in fname.
	...
	path and its position is returned, or -1 returned if not found. */
