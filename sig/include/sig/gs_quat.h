...
{  public :
   ...
   public :
	...
	/*! A null rotation represented as a rotation around axis (1,0,0)
		of angle 0, which generates the internal representation (1,0,0,0). */
   ...
   public :
	...
	/*! Initializes GsQuat extracting the rotation in line-major matrix m.
	...
	GsQuat ( const GsMat& m ) { set(m); }

	/*! Set a random quaternion, using a uniform distribution method */
	void setrandom ();

	/*! Returns a random quaternion using method setrandom() */
	static inline GsQuat random () { GsQuat q; q.setrandom(); return q; }
	...
	/*! Defines GsQuat from the line-major rotation in the given rotation matrix. */
	void set ( const GsMat& m );

	/*! Returns in m the equivalent line-major transformation, with translation in m set to 0. */
	GsMat& get ( GsMat& m ) const;
	...
	/*! Input data from "axis x y z ang a" format, a in degrees.
		Keywords "axis" and "ang" are optional.
		Additional format handled:
		"xzy x y z" for a rotation specified by XZY (and only XZY) Euler angles*/
	friend GsInput& operator>> ( GsInput& in, GsQuat& q );
};
...
/*! Converts the rotation in the given line-major rotation matrix to quaternion format. */
void mat2quat ( const GsMat& m, GsQuat& q );

/*! Equivalent to quat2mat(q,m), but retrieves only the 3x3 portion
...
/*! Converts the rotation encoded in the quaternion q to the line-major
	transformation matrix in m, with translation in m set to 0. */
...
{	mat2quat ( m, q ); t.set ( m.e14, m.e24, m.e34 ); }
